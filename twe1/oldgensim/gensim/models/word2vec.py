...
from oldgensim.gensim import utils, matutils  # utility fnc for pickling, common scipy operations etc
...
try:
    ...
    from oldgensim.gensim_addons.models.word2vec_inner import train_sentence_sg, train_sentence_cbow, FAST_VERSION, train_sentence_topic
except ImportError:
...
class Word2Vec(utils.SaveLoad):
    ...
    def save_wordvector(self, filename):
        with open(filename,"w") as f:
            title = str(len(self.vocab)) + " " + str(self.layer1_size)
            print >>f, title
            for w in self.vocab:
                v = self[w]
                now_line = str(w)
...
class CombinedSentence(object):
    ...
    def __iter__(self):
        for words, topics in zip(Text8Corpus(self.word_filename), Text8Corpus(self.topic_filename)):
            yield [(u,int(v)) for (u,v) in zip(words, topics)]
    """
    def __iter__(self):
        try:
            # Assume it is a file-like object and try treating it as such
            # Things that don't have seek will trigger an exception
            self.word_filename.seek(0)
            for line in self.word_filename:
                yield utils.to_unicode(line).split()
        except AttributeError:
            with utils.smart_open(self.word_filename) as word:
                for line in word:
                    yield line.strip().split()
    """
    ...
    def __iter__(self):
        # the entire corpus is one gigantic line -- there are no sentence marks at all
        # so just split the sequence of tokens arbitrarily: 1 sentence = 1000 tokens
        sentence, rest_word, max_sentence_length = [], b'', 1000
        rest_topic = b''
        #with utils.smart_open(self.word_filename) as fin:
        with utils.smart_open(self.topic_filename) as topic, utils.smart_open(self.word_filename) as word:
            while True:
                #text = rest + fin.read(8192)  # avoid loading the entire file (=1 line) into RAM
                text_word = rest_word + word.read(8192)
                text_topic = rest_topic + topic.read(8192)
                #topic.read(8192))

                if text_word == rest_word:  # EOF
                    sentence.extend(zip(rest_word.split(), rest_topic.split()))  # return the last chunk of words, too (may be shorter/longer)
                    if sentence:
                        yield sentence
                    break
                last_token = text_word.rfind(b' ')
                # the last token may have been split in two... keep it for the next iteration
                if last_token >= 0:
                    words_word, rest_word = (utils.to_unicode(text_word[:last_token]).split(), text_word[last_token:].strip())
                    words_topic, rest_topic = (utils.to_unicode(text_topic[:last_token]).split(), text_topic[last_token:].strip())
                else:
                    words_word, rest_word = ([], text_word)
                    words_topic, rest_topic = ([], text_topic)
                sentence.extend(zip(words_word, words_topic))
                while len(sentence) >= max_sentence_length:
                    yield sentence[:max_sentence_length]
                    sentence = sentence[max_sentence_length:]
        ...
        with utils.smart_open(self.topic_filename) as topic, utils.smart_open(self.word_filename) as word:
            for line1, line2 in zip(word, topic):
                ...
                topics = [int(t) for t in line2.strip().split()]
                #yield [(it1, int(it2)) for (it1, it2) in zip(words, topics)]
                y = [(it1, it2) for (it1, it2) in zip(words, topics)]
                #print(len(words))
                #print(len(topics))
                print("-----------")
                print(len(zip(words, topics)))
                #yield [(a,b) for (a,b) in zip(words, topics)]
                yield line1.strip().split()
    """
